...
/**
 * REST controller for managing {@link com.tickety.domain.Artist}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ArtistResource {
    ...
    public ResponseEntity<Artist> updateArtist(@PathVariable(value = "id", required = false) final Long id, @RequestBody Artist artist)
        ...
        Optional<Artist> result = artistRepository
            ...
            .map(existingArtist -> {
                ...
                if (artist.getInfo() != null) {
                    existingArtist.setInfo(artist.getInfo());
                }
                if (artist.getSocialMedia() != null) {
                    existingArtist.setSocialMedia(artist.getSocialMedia());
                }
