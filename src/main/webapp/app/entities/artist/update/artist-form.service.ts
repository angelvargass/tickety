...
type ArtistFormGroupContent = {
  ...
  info: FormControl<IArtist['info']>;
  socialMedia: FormControl<IArtist['socialMedia']>;
  event: FormControl<IArtist['event']>;
};
...
export class ArtistFormService {
  createArtistFormGroup(artist: ArtistFormGroupInput = { id: null }): ArtistFormGroup {
    ...
    return new FormGroup<ArtistFormGroupContent>({
      ...
      info: new FormControl(artistRawValue.info),
      socialMedia: new FormControl(artistRawValue.socialMedia),
      event: new FormControl(artistRawValue.event),
    });
  }
